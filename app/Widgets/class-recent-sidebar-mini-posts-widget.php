...
class Recent_Sidebar_Mini_Posts_Widget extends \WP_Widget
{
	...
	public function __construct() {
		$widget_ops = array(
			'classname'                   => 'widget_recent_mini_posts',
			'description'                 => __( 'Your site&#8217;s most recent Posts.' ),
			'customize_selective_refresh' => true,
			'show_instance_in_rest'       => true,
		);
		parent::__construct( 'recent-mini-posts', __( 'Recent Mini Posts' ), $widget_ops );
		$this->alt_option_name = 'widget_recent_mini_posts';
	}
	...
	public function widget( $args, $instance ) {
        ...
        <div>
			<?php foreach ( $r->posts as $recent_post ) : ?>
				...
				$aria_current = '';
                if ( $show_author ) :
                    $author_image = get_avatar_url(get_the_author_meta('ID'), array('size' => 48));
                    $author_link = get_author_posts_url(get_the_author_meta('ID'));
                endif;

                if ( $show_featured_image ) :
                    $post_image = get_the_post_thumbnail_url($recent_post->ID);
                endif;
                ...
                <article class="mini-post">
                    <header>
					    ...
					    <?php endif; ?>
                        <?php if ( $show_author ) : ?>
                            <a href="<?php echo esc_url($author_link) ?>" class="author">
                                <img src="<?php echo esc_url($author_image) ?>" alt="<?php echo get_the_author(); ?>" width="48" height="48" />
                            </a>
					    <?php endif; ?>
                    </header>
